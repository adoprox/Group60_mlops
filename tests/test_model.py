...
@pytest.mark.skipif(
    not os.path.exists(_PROJECT_ROOT + "/toxic_comments"), reason="Config files not found"
)
...
def test_model_output_shape(model, test_loader):
    ...
    assert outputs.logits.shape == (input_ids.size(0), model.model.num_labels), \
...
def test_model_label_prediction(model, test_loader):
    ...
    assert set(predicted_labels.flatten().tolist()) <= {0, 1}, \
...
def test_predictions_cuda_consistency(model, test_loader):
    ...
    assert torch.equal(outputs_cpu.logits.cpu(), outputs_gpu.logits.cpu()), "CPU and GPU model predictions are not consistent"
