...
def predict_user_input(config):
    ...
    if len(sys.argv) != 2:
        raise ValueError(
            "Wrong number of Arguments! use the format python ../predict_model.py '+text=text_to_classify'"
        )
    user_input = [config.text]

    # load the model
    model = ToxicCommentClassifier.load_from_checkpoint(
        "models/bert_toxic_classifier_logs/version_16/checkpoints/epoch=0-step=125.ckpt"
    )
